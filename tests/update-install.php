...
function reset_install(): array {
    ...
    signed_assets('0.2.5', str_replace("const PS_VERSION = '" . PS_VERSION . "';", "const PS_VERSION = '0.2.5';", $source));
    return ['version' => '0.2.5', 'revision' => $state['revision']];
}
...
try {
    ...
    $input = reset_install(); $assets[asset_url('0.2.5', 'update.sig')] = base64_encode(str_repeat('x', 64)); rejected($input, 'Forged signature');
    check(!isset(ps_load()['update_pending']), 'A failed download clears its reservation');
    $input = reset_install(); $assets[asset_url('0.2.5', 'update.json')] .= ' '; rejected($input, 'Tampered manifest');
    $input = reset_install(); $assets[asset_url('0.2.5', 'sitefren.php')] .= 'tampered'; rejected($input, 'Tampered release bytes');
    $input = reset_install(); signed_assets('0.2.5', $assets[asset_url('0.2.5', 'sitefren.php')], ['schema' => 2]); rejected($input, 'Unsupported storage schema');
    $input = reset_install(); signed_assets('0.2.5', $assets[asset_url('0.2.5', 'sitefren.php')], ['php_min' => '99.0.0']); rejected($input, 'Unsupported PHP version');
    $input = reset_install(); signed_assets('0.2.5', "<?php const PS_VERSION = '0.2.5'; broken syntax !"); rejected($input, 'Signed but invalid PHP');
    ...
    $url = asset_url('0.2.5', 'sitefren.php'); $assets['https://release-assets.githubusercontent.com/fixture'] = $assets[$url];
    $assets[$url] = ['https://release-assets.githubusercontent.com/fixture'];
    $result = ps_install_update($input);
    check($result['updated'] && $result['version'] === '0.2.5', 'Signed official redirect installs successfully');
    $after = ps_load();
    ...
    check($status === 0 && implode('', $output) === '0.2.5', 'A fresh PHP process boots the replacement with the preserved state');
