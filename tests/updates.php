...
$calls = 0;
$http = 200;
$reply = '[{"tag_name":"v0.2.3","draft":false,"prerelease":true}]';
...
try {
    ...
    check(ps_release_version([
        ...
        ['tag_name'=>'<script>'], ['tag_name'=>'v0.2.3-rc.1','prerelease'=>true],
    ]) === '0.2.3-rc.1', 'Version ordering includes alpha releases and excludes drafts and malformed tags');
    check(ps_release_version([]) === null, 'An empty feed has no advertised version');
    $result = ps_check_updates(false);
    check($result['available'] && $result['version'] === '0.2.3' &&
