...
(async () => {
  ...
  try {
    ...
    check(await page.locator('.sidebar').isHidden() && await page.locator('.workbench').isVisible(), 'Mobile starts with the website workspace instead of a long stacked chat');
    ...
    await page.locator('#mobileSiteBtn').click(); await page.locator('#mobileChatBtn').click();
    check(await page.locator('#prompt').inputValue() === 'Keep my unsent idea', 'Switching mobile views preserves the unsent prompt');
    await page.locator('#mobileSiteBtn').click();
    ...
    check(await page.locator('.mobile-switch').isHidden(), 'Editing gives the preview more space and prevents switching away mid-edit');
