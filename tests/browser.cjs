...
(async()=>{
 ...
 if(process.env.POCKET_TEST_TRANSPORT==='1'){
  ...
  const assets={};
  for(const version of ['0.2.3','0.2.4']){
   const code=source.replace("const PS_VERSION = '0.2.3';",`const PS_VERSION = '${version}';`);
 ...
 try{
  ...
  check((await page.locator('#versionBadge').innerText())==='ALPHA 0.2.3'||(await page.locator('#versionBadge').innerText())==='Alpha 0.2.3','The shipped version is identified as Alpha 0.2.3');
  ...
  if(process.env.POCKET_TEST_TRANSPORT==='1'){
   ...
   check((await page.locator('#updateAvailable').innerText()).includes('0.2.4'),'A newer published release shows an update notice');
  }
  ...
  check(adBounds.x>=tabBounds.x+tabBounds.width&&adBounds.x+adBounds.width<=controlBounds.x&&adBounds.width<=440,'The compact ad fits between tabs and preview controls');
  ...
  if(process.env.POCKET_TEST_TRANSPORT==='1'){
   ...
   check(fs.readFileSync(path.join(root,'sitefren.php'),'utf8').includes("const PS_VERSION = '0.2.3';"),'Cancel leaves the installed version untouched');
   ...
   await page.waitForFunction(()=>document.querySelector('#versionBadge').textContent.includes('0.2.4'));
