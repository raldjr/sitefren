...
(async()=>{
 ...
 if(process.env.POCKET_TEST_TRANSPORT==='1'){
  ...
  const assets={};
  for(const version of ['0.2.5','0.2.6']){
   const code=source.replace("const PS_VERSION = '0.2.5';",`const PS_VERSION = '${version}';`);
 ...
 try{
  ...
  check((await page.locator('#versionBadge').innerText())==='ALPHA 0.2.5'||(await page.locator('#versionBadge').innerText())==='Alpha 0.2.5','The shipped version is identified as Alpha 0.2.5');
  ...
  check(await page.locator('.sidebar').isVisible()&&await page.locator('.workbench').isVisible()&&await page.locator('.workspace-switch').isHidden(),'Desktop shows chat and website side by side');
  await page.locator('#prompt').fill('Keep this idea while I look around');
  await page.setViewportSize({width:390,height:844});await page.locator('#editorWorkspaceBtn').click();await page.locator('#chatWorkspaceBtn').click();await page.setViewportSize({width:1440,height:1000});
  if(process.env.SCREENSHOT_PATH)await page.screenshot({path:process.env.SCREENSHOT_PATH+'.chat.png',fullPage:true});

  check(await page.locator('#prompt').inputValue()==='Keep this idea while I look around','Resizing between mobile and desktop preserves unsent prompts');
  ...
  if(process.env.POCKET_TEST_TRANSPORT==='1'){
   ...
   check((await page.locator('#updateAvailable').innerText()).includes('0.2.6'),'A newer published release shows an update notice');
  }
  await page.locator('#chatDemoBtn').click();
  ...
  await page.locator('#desktopBtn').click();await page.waitForFunction(()=>document.querySelector('#previewShell').getBoundingClientRect().width>600);
  ...
  for(const width of [721,900,1440]) {
   ...
   const chat=await page.locator('.sidebar').boundingBox();const editor=await page.locator('.workbench').boundingBox();
   check(chat.x+chat.width<=editor.x&&await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),'Split workspace fits '+width+'px without overlap or page overflow');
  }
  ...
  await page.locator('#codeEditor').fill(modified);await page.setViewportSize({width:390,height:844});await page.locator('#chatWorkspaceBtn').click();await page.locator('#editorWorkspaceBtn').click();await page.setViewportSize({width:1440,height:1000});
  ...
  if(process.env.POCKET_TEST_TRANSPORT==='1'){
   ...
   check(fs.readFileSync(path.join(root,'sitefren.php'),'utf8').includes("const PS_VERSION = '0.2.5';"),'Cancel leaves the installed version untouched');
   ...
   await page.waitForFunction(()=>document.querySelector('#versionBadge').textContent.includes('0.2.6'));
