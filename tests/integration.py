...
with tempfile.TemporaryDirectory(prefix='pocket-http-') as tmp:
    ...
    try:
        ...
        check(request('update_now',{'version':'0.2.1'})[0]==401,'Installing updates requires authentication')
        ...
        check(request('update_now',{'version':'0.2.1'},token=False)[0]==403,'Installing updates requires CSRF')
        ...
        if fixture:
            check(update['available'] and update['version']=='0.2.1' and update['url']=='https://github.com/raldjr/sitefren/releases','New alpha releases return a fixed official download link')
