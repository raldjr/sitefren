...
with tempfile.TemporaryDirectory(prefix='pocket-http-') as tmp:
    ...
    try:
        ...
        check(request('update_now',{'version':'0.2.3'})[0]==401,'Installing updates requires authentication')
        ...
        check(request('update_now',{'version':'0.2.3'},token=False)[0]==403,'Installing updates requires CSRF')
        check(request('update_now',verb='GET')[0]==405,'GET cannot install an update')
        ...
        if fixture:
            check(update['available'] and update['version']=='0.2.3' and update['url']=='https://github.com/raldjr/sitefren/releases','New alpha releases return a fixed official download link')
